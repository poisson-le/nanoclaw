...
import type { AgentGroupMember, UserRole, User, UserDm } from './types.js';
...
function collectAgentGroups() {
  return getAllAgentGroups().map((g) => {
    ...
    const members = getMembers(g.id).map((m: AgentGroupMember) => {
      const user = getUser(m.user_id);
      return { ...m, display_name: user?.display_name ?? null };
    });
    const admins = getAdminsOfAgentGroup(g.id).map((a: UserRole) => {
...
function collectUsers() {
  return getAllUsers().map((u: User) => {
    ...
    let privilege = 'none';
    if (roles.some((r: UserRole) => r.role === 'owner')) privilege = 'owner';
    else if (roles.some((r: UserRole) => r.role === 'admin' && !r.agent_group_id)) privilege = 'global_admin';
    else if (roles.some((r: UserRole) => r.role === 'admin')) privilege = 'admin';
    ...
    return {
      ...
      roles,
      memberships,
      dmChannels: dms.map((d: UserDm) => ({ channel_type: d.channel_type })),
