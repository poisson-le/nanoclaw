...
registerChannelAdapter('wechat', {
  factory: () => {
    ...
    async function ensureLoggedIn(): Promise<WeChatClient> {
      ...
      const c = new WeChatClient({ channelVersion: '2.4.1' });
      const result = await c.login({
        ...
        onStatus: (status) => {
          log.info('WeChat QR status change', { status });
        },
      });
      log.info('WeChat login result', { connected: result.connected, message: result.message });
